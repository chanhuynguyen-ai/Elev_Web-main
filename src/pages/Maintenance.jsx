...
export default function Maintenance() {
  ...
  const runLLMQuery = async () => {
    ...
    try {
      const data = await api.chat(llmInput, { session_id: agentSessionId, employee_id: empCode, employee_name: empCode });
      setLlmOutput(data.answer || 'Không có phản hồi.');
      setLlmAgentData(data);
      setAgentSessionId(data.session_id || agentSessionId);
      if (data.requires_human) showToast('Agent đề xuất escalte cho kỹ thuật viên khác hoặc trực ca.');
    } catch (err) {
    ...
    <div>
      ...
      <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        <div className="card llm-console-card">
          ...
          <div className="llm-quick-row">
            {[
              'Tóm tắt lỗi nổi bật hôm nay',
              'Lỗi nào xuất hiện nhiều nhất?',
              'Trạng thái thang máy hiện tại',
              'Những cảnh báo cần ưu tiên xử lý',
            ].map((prompt) => (
              <button key={prompt} className="lookup-tab" onClick={() => setLlmInput(prompt)}>{prompt}</button>
            ))}
          </div>

          <div className="llm-output llm-answer-box">{llmOutput}</div>

          <div className="inline llm-console-input-row">
            <input
              style={{ flex: 1 }}
              value={llmInput}
      ...
      <div className="maint-grid" style={{ marginTop: 14 }}>
        ...
        <div className="card">
          ...
          <textarea style={{ width: '100%', height: 80, fontFamily: 'monospace', padding: 8, marginBottom: 8 }} value={sqlQuery} onChange={(e) => setSqlQuery(e.target.value)} />
        ...
        <div className="lock-grid">
          {ALL_FLOORS.map((f) => (
            <div key={f} className="lock-item">
              <button className={`lock-btn ${lockedFloors.includes(f) ? 'locked' : 'unlocked'}`} onClick={() => toggleFloorLock(f)}>
